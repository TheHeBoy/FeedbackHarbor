<template>
  ...
  >
    ...
    <div v-if="!resetPasswdShow">
      ...
      <el-form-item>
        <XButton type="primary" title="通过邮箱找回" class="w-full" @click="checkUsername" />
      </el-form-item>
    </div>
    <div v-else>
      <!-- 密码 -->
    ...
    <el-form-item>
      <XButton title="返回" class="w-full" @click="handleBackLogin()" />
    </el-form-item>
  </el-form>
</template>
...
const getShow = computed(() => unref(getLoginState) === LoginStateEnum.FORGETP_ASSWDFORM);
const formRef = ref();
  ...
  username: [
    { required: true, message: '请输入用户名', trigger: 'blur' },
    {
      validator: async (rule, value, callback) => {
        if (value) {
          const result = await AuthApi.checkUsername(registerForm.value.username);
...
const generateCaptcha = async () => {
  await AuthApi.sendResetPasswdMailCaptcha(registerForm.value.mail);
...
const submitForm = () => {
  formRef.value?.validate(async (valid: any) => {
    if (valid) {
      loading.value = true;
      try {
        await AuthApi.resetPasswd(registerForm.value);
