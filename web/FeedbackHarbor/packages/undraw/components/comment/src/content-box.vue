    ...
    <div class="comment-primary">
      <div class="comment-main">
          ...
          <u-fold unfold>
            ...
            <div class="imgbox" style="display: flex">
              <template v-for="(url, index) in imgList" :key="index">
                <ElImage :src="url" style="height: 72px; padding: 8px 4px" lazy :preview-src-list="imgList"
                  :initial-index="index"></ElImage>
              </template>
            </div>
          </u-fold>
        </div>
        <div class="flex w-full justify-between">
          ...
          <div class="action-box select-none">
            ...
            <div ref="btnRef" class="item" :class="{ active: state.active }" @click="reply">
              <u-icon>
                <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1320" width="200"
                  height="200">
                  <path
            ...
            <div class="item" @click="like(str(data.id))">
              <u-icon v-if="user.likeIds.map(String).indexOf(str(data.id)) == -1">
                <svg t="1650360973068" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                  p-id="1168" width="200" height="200">
                  <path
        ...
        <div v-if="state.active">
          <InputBox ref="commentRef" :parent-id="str(id)" :placeholder="`回复 @${data.user.username}...`" :reply="data"
            content-btn="发布" style="margin-top: 12px" @hide="hide" @close="state.active = false" />
        </div>
      </div>
      <!-- 回复列表 -->
      <slot></slot>
